...
router.post('/register', async (req, res) => {
  try {
    ...
    res.status(500).json({ hata: 'Sunucu tarafında kayıt işlemi sırasında bir hata oluştu' });
  }
});
...
router.post('/login', async (req, res) => {
  try {
    ...
    res.status(500).json({ hata: 'Sunucu tarafında giriş işlemi sırasında bir hata oluştu' });
  }
});
